...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Optional: You can validate/sanitize here

    $stmt = $conn->prepare("INSERT INTO feedback (name, phone_number, msg) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $name, $phone, $msg);
...
<head>
  ...
  <link href="../CC_Video_Advertisining/css/feedback.css" rel="stylesheet">
</head>
<body>
  <!-- Navbar -->
  ...
  <main class="container my-5">
    <div class="p-5 mx-auto rounded-3" style="max-width: 600px; background-color: #a72020; color: white;">
      <h2 class="text-center mb-4">FeedBack</h2>
        <form action="feedback.php" method="POST">
  <input type="text" name="name" class="form-control mb-3" placeholder="User Name" required>
  <input type="text" name="phone_number" class="form-control mb-3" placeholder="Phone Number" required>
  <input type="email" name="email" class="form-control mb-3" placeholder="Email" required>
  <div class="mb-3">
    <label for="message" class="form-label">Message</label>
    <textarea name="msg" id="message" class="form-control bg-light text-dark" rows="5" required></textarea>
  </div>
  <div class="d-flex justify-content-center gap-3">
    <button type="submit" class="btn btn-dark px-4 rounded-pill">Submit</button>
    <a href="../CC_Video_Advertisining/review.php" class="btn btn-dark px-4 rounded-pill">Review</a>
  </div>
</form>
</main>
